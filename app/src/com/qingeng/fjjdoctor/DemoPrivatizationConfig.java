...
public class DemoPrivatizationConfig {
    ...
    private static void autoAdjust(@NonNull ServerAddresses addresses) {
        ...
        //如果配置走新协议
        if (addresses.handshakeType == NimHandshakeType.V0) {
            //没有配置parta，却配置了module，则认为是走RSA+RC4，将module复制到parta
            if (addresses.negoKeyEncaKeyParta == null && addresses.module != null) {
                addresses.negoKeyNeca = AsymmetricType.RSA;
                addresses.commEnca = SymmetryType.RC4;
                addresses.negoKeyEncaKeyParta = addresses.module;
                addresses.negoKeyEncaKeyPartb = "10001";
            }
            addresses.module = null;
        }
        //走老协议
        else if (addresses.handshakeType == NimHandshakeType.V1) {
            if (addresses.module == null && addresses.negoKeyEncaKeyParta != null) {
                addresses.module = addresses.negoKeyEncaKeyParta;
            }
            addresses.negoKeyEncaKeyParta = null;
            addresses.negoKeyEncaKeyPartb = null;
        }
    }
