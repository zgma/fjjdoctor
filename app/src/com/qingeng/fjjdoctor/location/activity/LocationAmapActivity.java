package com.qingeng.fjjdoctor.location.activity;
...
import com.qingeng.fjjdoctor.adapter.MapAddressAdapter;
import com.qingeng.fjjdoctor.location.helper.NimGeocoder;
import com.qingeng.fjjdoctor.location.helper.NimLocationManager;
import com.qingeng.fjjdoctor.location.helper.NimLocationManager.NimLocationListener;
import com.qingeng.fjjdoctor.location.model.NimLocation;
...
public class LocationAmapActivity extends UI implements OnCameraChangeListener, OnClickListener, NimLocationListener, PoiSearch.OnPoiSearchListener,MapAddressAdapter.Listener, Inputtips.InputtipsListener {
    ...
    private void initView() {
        ...
        edit_input = findView(R.id.edit_input);
        rcl_list = findView(R.id.rcl_list);
        LinearLayoutManager linearLayoutManager = new LinearLayoutManager(this);
        linearLayoutManager.setOrientation(linearLayoutManager.VERTICAL);
        rcl_list.setLayoutManager(linearLayoutManager);
        mapAddressAdapter = new MapAddressAdapter();
        mapAddressAdapter.setListener(this);
        rcl_list.setAdapter(mapAddressAdapter);
            ...
            public boolean onEditorAction(TextView v, int actionId, KeyEvent event) {
                if (actionId == EditorInfo.IME_ACTION_SEARCH) {
                    ...
                    if (actionId == EditorInfo.IME_ACTION_SEARCH) {
                        ...
                        searchInput(edit_input.getText().toString().trim());
                        return true;
                    }
                    // 在这里写搜索的操作,一般都是网络请求数据
                    return true;
    ...
    public void onLocationChanged(NimLocation location) {
        if (location != null && location.hasCoordinates()) {
            ...
            default_city = location.getCityName();
            if (locating) {
                locating = false;
                locationAddressInfo(cacheLatitude, cacheLongitude, cacheAddressInfo);
            }
    ...
    private void searchPoi(String s1,String s2){
    ...
    private void searchInput(String s1){
    ...
    public void onPoiSearched(PoiResult result, int i) {
        ...
        for (int j = 0; j < pois .size(); j++) {
            ...
            if (i == 0) cacheAddressInfo = poi.getTitle();
            AddressBean addressBean = new AddressBean();
            addressBean.setName(poi.getTitle());
            addressBean.setAddress(poi.getSnippet());
            addressBean.setLat(poi.getLatLonPoint().getLatitude());
            addressBean.setLon(poi.getLatLonPoint().getLongitude());
            addressBeans.add(addressBean);
        }
        mapAddressAdapter.setAddressBeans(addressBeans);
    ...
    public void onItemClick(AddressBean addressBean) {
        locationAddressInfo(addressBean.getLat(), addressBean.getLon(), addressBean.getName());
    }
    ...
    public void onGetInputtips(List<Tip> list, int i) {
        ...
        if (addressBeans.size()>0){
            locationAddressInfo(addressBeans.get(0).getLat(), addressBeans.get(0).getLon(), addressBeans.get(0).getName());
        }
    }
}
