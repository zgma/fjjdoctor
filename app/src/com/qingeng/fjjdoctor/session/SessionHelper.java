package com.qingeng.fjjdoctor.session;
...
import com.qingeng.fjjdoctor.contact.activity.RobotProfileActivity;
import com.qingeng.fjjdoctor.contact.activity.UserProfileActivity2;
import com.qingeng.fjjdoctor.main.helper.MessageHelper;
import com.qingeng.fjjdoctor.session.action.AVChatAction;
import com.qingeng.fjjdoctor.session.action.AckMessageAction;
import com.qingeng.fjjdoctor.session.action.CardAction;
import com.qingeng.fjjdoctor.session.action.FileAction;
import com.qingeng.fjjdoctor.session.action.GuessAction;
import com.qingeng.fjjdoctor.session.action.RTSAction;
import com.qingeng.fjjdoctor.session.action.RedPacketAction;
import com.qingeng.fjjdoctor.session.action.SnapChatAction;
import com.qingeng.fjjdoctor.session.action.TeamAVChatAction;
import com.qingeng.fjjdoctor.session.action.TipAction;
import com.qingeng.fjjdoctor.session.action.TransferAction;
import com.qingeng.fjjdoctor.session.activity.AckMsgInfoActivity;
import com.qingeng.fjjdoctor.session.activity.MessageHistoryActivity;
import com.qingeng.fjjdoctor.session.activity.MessageInfoActivity;
import com.qingeng.fjjdoctor.session.extension.CardAttachment;
import com.qingeng.fjjdoctor.session.extension.CustomAttachParser;
import com.qingeng.fjjdoctor.session.extension.CustomAttachment;
...
import com.qingeng.fjjdoctor.session.search.SearchMessageActivity;
import com.qingeng.fjjdoctor.session.viewholder.MsgViewHolderAVChat;
import com.qingeng.fjjdoctor.session.viewholder.MsgViewHolderCard;
import com.qingeng.fjjdoctor.session.viewholder.MsgViewHolderDefCustom;
import com.qingeng.fjjdoctor.session.viewholder.MsgViewHolderFile;
import com.qingeng.fjjdoctor.session.viewholder.MsgViewHolderGuess;
import com.qingeng.fjjdoctor.session.viewholder.MsgViewHolderMultiRetweet;
import com.qingeng.fjjdoctor.session.viewholder.MsgViewHolderRTS;
import com.qingeng.fjjdoctor.session.viewholder.MsgViewHolderSnapChat;
import com.qingeng.fjjdoctor.session.viewholder.MsgViewHolderSticker;
import com.qingeng.fjjdoctor.session.viewholder.MsgViewHolderTip;
...
import com.qingeng.fjjdoctor.session.viewholder.MsgViewHolderTransfer;
...
public class SessionHelper {
    ...
    private static SessionCustomization getP2pCustomization() {
        if (p2pCustomization == null) {
            ...
            //            p2pCustomization.backgroundUri = "android.resource://com.chen.fastchat/drawable/bk"
...
//            TeamAVChatProfile.sharedInstance().registerObserver(true); 音频注释
            ArrayList<BaseAction> actions = new ArrayList<>();
    ...
    private static void setSessionListener() {
        SessionEventListener listener = new SessionEventListener() {
            ...
            public void onAvatarClicked(Context context, IMMessage message) {
             ...
             /*   if (message.getMsgType() == MsgTypeEnum.robot && message.getDirect() == MsgDirectionEnum.In) {
                ...
                if (message.getSessionType() == SessionTypeEnum.Team ){
                    String data =  UserPreferences.getLastTeamCanP2P();
                    String[] strings = new String[2];
                    if (data.contains("_")){
                        strings = data.split("_");
                        if (strings.length==2 && strings[0].equals(message.getSessionId()) && strings[1].equals("false")){
                            return;
                        }
                    }
            ...
            public void onAvatarLongClicked(Context context, IMMessage message) {
                ...
                if (!message.getFromAccount().equals(AppPreferences.getAccId())){
                    EventBus.getDefault().post("@_"+message.getFromAccount()+"_"+message.getFromNick());
                }
            }
