package com.qingeng.fjjdoctor.team;
...
public class TeamCreateHelper {
    ...
    public static void createAdvancedTeam(final Context context, List<String> memberAccounts) {
        ...
        for (int i = 0; i < memberAccounts.size(); i++) {
            if (i==memberAccounts.size()-1){
                ids.append(memberAccounts.get(i));
            }else {
                ids.append(memberAccounts.get(i)+",");
            }
        }
...
/*
        ...
        HttpClient.createTeam(ids.toString(), new HttpInterface() {
            @Override
            public void onSuccess(int requestCode, BaseResponseData baseResponseData) {
                String json = JSON.toJSONString(baseResponseData.getData());
        ...
        }, RequestCommandCode.CREATE_TEAM);
    }
     ...
     * 群创建成功回调
     */
    private static void onCreateSuccess(final Context context, String teamId) {
        ...
        new Handler(context.getMainLooper()).postDelayed(new Runnable() {
            @Override
            public void run() {
                SessionHelper.startTeamSession(context, teamId); // 进入创建的群
            }
        }, 50);
    }
}
