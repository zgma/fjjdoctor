...
import com.acker.simplezxing.util.BitmapUtil;
...
public final class CaptureActivity extends Activity implements SurfaceHolder.Callback, SensorEventListener {
    ...
    public static final int REQUEST_CODE = 0xF0F1;
    ...
    private void bundleSetting(Bundle bundle) {
        ...
        needScanHintText = bundle.getBoolean(KEY_NEED_SCAN_HINT_TEXT, VALUE_SCAN_HINT_TEXT);
    ...
    protected void onResume() {
        ...
        viewfinderView.setOnLightClickListener(new ViewfinderView.OnLightClickListener() {
            @Override
            public void onLightClick() {
                if (flashlightMode == VALUE_FLASHLIGHT_ON) {
                    if (cameraManager != null) {
                        cameraManager.setTorch(false);
                        flashlightMode = VALUE_FLASHLIGHT_OFF;
                    }
                } else {
                    if (cameraManager != null) {
                        cameraManager.setTorch(true);
                        flashlightMode = VALUE_FLASHLIGHT_ON;
                    }
                }
            }
        });
    ...
    public void onSensorChanged(SensorEvent event) {
        if (event.sensor.getType() == Sensor.TYPE_LIGHT) {
            ...
            if (light_strength < 5) {
                viewfinderView.setShowLightImage(true);
            } else {
                viewfinderView.setShowLightImage(false);
            }
        }
    }
    ...
    private void handleAlbumPic(Intent data) {
        ...
        runOnUiThread(new Runnable() {
            ...
            public void run() {
                ...
                mProgress.dismiss();
                if (result != null) {
                    returnResult(RESULT_OK, result.getText());
                } else {
