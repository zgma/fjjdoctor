...
public class TeamMessageActivity extends BaseMessageActivity {
    ...
    private void requestTeamInfo() {
        ...
        HttpClient.groupDetail(sessionId, new HttpInterface() {
            @Override
            public void onSuccess(int requestCode, BaseResponseData baseResponseData) {
                ...
                boolean isManage = groupUser.getCurrentUserIdentity().equals("群主") || groupUser.getCurrentUserIdentity().equals("管理员");
                UserPreferences.setLastTeamCanP2P(!waHuHighGroup.getPrivateMode().equals("不允许私聊") || isManage? sessionId + "_true" : sessionId + "_false");
                UserPreferences.setLastTeamRemoveMemberNotice(waHuHighGroup.getSubstractMode().equals("减员提醒") ? sessionId + "_true" : sessionId + "_false");
                showNotice();

                if(team !=null && waHuHighGroup!=null){
                    setTitle(team == null ? sessionId : team.getName() + "(" + waHuHighGroup.getHighGroups().size() + "人)");
                }
    ...
    private void onRequestTeamInfoFailed() {
        ToastHelper.showToast(TeamMessageActivity.this, "获取群组信息失败，你已离开或者群已解散!");
        finish();
    }
    ...
    private void showNotice() {
        if (waHuHighGroup.getNoticeMode().equals("强提醒") && !TextUtils.isEmpty(waHuHighGroup.getAnnouncement()) && !isShow) {
            EasyAlertDialogHelper.showOneButtonDiolag(TeamMessageActivity.this, "群公告", waHuHighGroup.getAnnouncement(),
                    getString(com.netease.nim.uikit.R.string.ok),
                    true, null);
            isShow = true;
        }

        fragment.setMyUserName(groupUser.getTalkName());
        fragment.setScreenNotify(waHuHighGroup.getScreenNotify()==1);
