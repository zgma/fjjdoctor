 ...
 */
public class MessageListPanelEx implements HttpInterface {
    ...
    @Subscribe
    public void messageRefresh(IMMessage message) {
        System.out.println("-===========================================");
        onMsgSend(message);
    }

    @Subscribe
    public void messageRefresh(EventBusMessage message) {
        if (message.getCode().equals(EventBusMessage.MESSAGE_REFRESH_LIST)){
            System.out.println("-=============MESSAGE_REFRESH_LIST==============================");
...
//            // resend的的情况，可能时间已经变化了，这里要重新检查是否要显示时间
    ...
    public void setChattingBackground2(Uri uri, int color) {
        if (uri != null) {
            if (uri.getScheme().equalsIgnoreCase("file") && uri.getPath() != null) {
                ivBackground.setImageBitmap(getBackground(uri.getPath()));
            } else if (uri.getScheme().equalsIgnoreCase("android.resource")) {
    ...
    private class MsgItemEventListener extends MsgAdapter.BaseViewHolderEventListener {
        ...
        // 长按菜单项--复制
        private void longClickItemCollect(final IMMessage item, CustomAlertDialog alertDialog, MsgTypeEnum msgType) {
//            if (msgType == MsgTypeEnum.text || (msgType == MsgTypeEnum.robot && item.getAttachment() != null && !((RobotAttachment) item.getAttachment()).isRobotSend())) {
            alertDialog.addItem("收藏", new CustomAlertDialog.onSeparateItemClickListener() {
                @Override
                public void onClick() {
                    String type = msgType == MsgTypeEnum.text ? "1" :
                            msgType == MsgTypeEnum.audio ? "2" :
                                    msgType == MsgTypeEnum.image ? "3" :
                                            msgType == MsgTypeEnum.video ? "4" : "7";
                    String filepath = null;
                    File file = null;
                    boolean fileIsDownload = false;
                    if (type.equals("1")) {
                        filepath = "";
                    } else {
                        String s2 = (String) JSONObject.parseObject(item.getAttachment().toJson(false)).get("path");
                        filepath = s2;
                        if (s2 != null)fileIsDownload = true;
                    }
                    if (fileIsDownload){
                        file  = new File(filepath);
                        CollectMessage(type, item, file);
                    }else {
                        getMessageFileDataAndCollect(type, item);
                    }
...
//                    NIMClient.getService(MsgService.class).revokeMessage(item).setCallback(new RequestCallback<Void>() {
    ...
    private void getMessageFileDataAndCollect(String type,IMMessage item) {
        JSONObject messageInfo = JSONObject.parseObject(item.getAttachment().toJson(false));
        String fileName = messageInfo.getString("md5")+"."+messageInfo.getString("ext");
        File file = new File(MainConstant.DOWNLOAD_DIRECTORY + fileName);
        if (file.exists()) {
            CollectMessage(type, item, file);
        } else {
            downloadMessageFile(messageInfo.getString("url"), file.getPath(),type, item);
        }
    }

    private void CollectMessage(String type,IMMessage item,File file){
        HttpClient.CollectMessage(type, item.getUuid(), item.getContent(),
                item.getSessionId(), file, new HttpInterface() {
                    @Override
                    public void onSuccess(int requestCode, BaseResponseData baseResponseData) {
                        ToastHelper.showToast(container.activity, "收藏成功");
                    }

                    @Override
                    public void onFailure(int requestCode, String message) {
                        ToastHelper.showToast(container.activity, "收藏失败");
                    }
                ...
                }, RequestCommandCode.ADD_FAVORITE_MSG);
    }

    private void downloadMessageFile(String url, String filePath,String type,IMMessage item) {
    ...
    public void onSuccess(int requestCode, BaseResponseData baseResponseData) {
        switch (requestCode) {
            case RequestCommandCode.GET_ONE_THEME:
