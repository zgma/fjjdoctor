...
public class HeadImageView extends AppCompatImageView {
    ...
    private void loadImage(final String url, final int defaultResId, final int thumbSize) {
        ...
        RequestOptions requestOptions = new RequestOptions().centerCrop().bitmapTransform(roundedCorners).placeholder(defaultResId)
                                                            .error(defaultResId).override(thumbSize,
                                                                                          thumbSize);
        Glide.with(getContext().getApplicationContext()).asBitmap().load(url!=null?url.split("\\?")[0]:"").apply(requestOptions)
             .into(this);
    }
    ...
    public void loadBuddyAvatar(Uri fromFile) {
        RequestOptions requestOptions = new RequestOptions().centerCrop().bitmapTransform(roundedCorners).placeholder(DEFAULT_AVATAR_RES_ID)
                .error(DEFAULT_AVATAR_RES_ID).override(DEFAULT_AVATAR_THUMB_SIZE,
                        DEFAULT_AVATAR_THUMB_SIZE);
        Glide.with(getContext().getApplicationContext()).asBitmap().load(fromFile).apply(requestOptions)
                .into(this);
    }


    public void loadImgForUrl(String url) {
        RequestOptions requestOptions = new RequestOptions().centerCrop().bitmapTransform(roundedCorners).placeholder(DEFAULT_AVATAR_RES_ID)
                .error(DEFAULT_AVATAR_RES_ID).override(DEFAULT_AVATAR_THUMB_SIZE,
                        DEFAULT_AVATAR_THUMB_SIZE);
    ...
    public void loadImgForId(int imgId) {
        RequestOptions requestOptions = new RequestOptions().centerCrop().bitmapTransform(roundedCorners).placeholder(DEFAULT_AVATAR_RES_ID)
                .error(DEFAULT_AVATAR_RES_ID).override(DEFAULT_AVATAR_THUMB_SIZE,
                        DEFAULT_AVATAR_THUMB_SIZE);
        Glide.with(getContext().getApplicationContext()).asBitmap().load(imgId).apply(requestOptions)
                .into(this);
    }
    public void loadTeamHead(String url) {
        int defaultResId = R.drawable.nim_avatar_group;
        RequestOptions requestOptions = new RequestOptions().bitmapTransform(roundedCorners).centerCrop().placeholder(defaultResId)
                .error(defaultResId).override(DEFAULT_AVATAR_THUMB_SIZE,
                        DEFAULT_AVATAR_THUMB_SIZE);
        Glide.with(getContext().getApplicationContext()).asBitmap().load(url).apply(requestOptions)
                .into(this);
    }
    ...
    public void loadImgForUrlWithCorners(String url,int roundingRadius) {
